...
def load_model(config: str, path: str):
    ...
    print(f"   ✅ {config} loaded | AUC: {val_auc}")
    return model, val_auc

try:
    models_dict["single_qubit"],    auc_sq = load_model("single_qubit",    "models/single_qubit_best.pth")
    models_dict["entanglement"],    auc_en = load_model("entanglement",    "models/entanglement_best.pth")
    models_dict["full_variational"],auc_fv = load_model("full_variational","models/full_variational_best.pth")
...
def root():
    return {
        "status": "🟢 Quantum Cancer Detection API is running!",
        "models": ["single_qubit", "entanglement", "full_variational"],
        "endpoints": {
            "predict_all":  "POST /predict",
            "predict_one":  "POST /predict/{model_name}",
            "health":       "GET  /health",
            "docs":         "GET  /docs"
        }
    }
...
async def predict_all(file: UploadFile = File(...)):
    """Run the mammogram image through all 3 quantum models and return combined results."""
    ...
    # Ensemble: average probability across all 3 models
...
async def predict_single(model_name: str, file: UploadFile = File(...)):
    """Run prediction with a specific model: single_qubit | entanglement | full_variational"""
    if model_name not in models_dict:
        raise HTTPException(
            status_code=404,
            detail=f"Model '{model_name}' not found. Choose from: {list(models_dict.keys())}"
        )
    if not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="File must be an image.")
